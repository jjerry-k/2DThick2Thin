import os, tqdm, datetime, random, argparse, csv
import cv2 as cv
import numpy as np
import pandas as pd
...
loss_dict = {'MSE': mse, 'GRAD': gradient_2d_loss, 'MSEGRAD': mse_grad_loss}
...
for img_name, lab_name in zip(train_img_list, train_lab_list):
    ...
    tmp_img = np.load(os.path.join(data_root, img_name))[..., np.newaxis]
    tmp_lab = np.load(os.path.join(data_root, lab_name))[..., np.newaxis]
...
val_img = np.load(os.path.join(data_root, img_name))[..., np.newaxis]
val_lab = np.load(os.path.join(data_root, lab_name))[..., np.newaxis]
...
G = COR_SR2D_res(residual_channel=residual_channels, 
             layer_activation='leaky_relu', 
             name='3D_SR_Gen')
D = dis2D_res(residual_channel=residual_channels, 
              name='3D_SR_Dis')
...
A.compile(optimizer=optimizers.Adam(lr=0.0001, epsilon=1e-8), loss=[loss_dict[args.loss.upper()], losses.binary_crossentropy], 
          loss_weights=[1, 1], metrics={'3D_SR_Gen_output_act':[gradient_2d_loss, psnr]})
print("==================================================\n")
...
for epoch in range(epochs):
    ...
    val_gen_tot = epoch_v_g_total/num_val_iter
    val_gen_sty = epoch_v_g_style/num_val_iter
    val_gen_ad = epoch_v_g_dis/num_val_iter
    val_mse = epoch_v_mse/num_val_iter
    val_grad = epoch_v_grad/num_val_iter
    val_psnr = epoch_v_psnr/num_val_iter
    
    mean_val_loss = val_gen_sty

    # Progressbar Update (Last Step)
    ...
    if mean_val_loss < top_gen_loss:
        ...
        if epoch == 0: 
            A.save_weights(os.path.join(ckpt_root, "%05d_%.4f_%.4f.h5"%(epoch+1, tr_gen_sty, val_gen_sty)))
            top_epoch = epoch
